...
RdmaClient::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::RdmaClient")
    ...
    .AddAttribute ("OmniDMAEnable",
                   "OmniDMAEnable",
                   UintegerValue (0),
                   MakeUintegerAccessor (&RdmaClient::m_omnidma_enable),
                   MakeUintegerChecker<uint8_t> ())
                   
    .AddAttribute ("OmniDMABitmapSize",
                    "OmniDMABitmapSize",
                    UintegerValue (0),
                    MakeUintegerAccessor (&RdmaClient::m_omnidma_bitmap_size),
                    MakeUintegerChecker<uint16_t> ())
...
void RdmaClient::StartApplication (void)
{
  ...
  rdma->AddQueuePair(m_size, m_pg, m_sip, m_dip, m_sport, m_dport, m_win, m_baseRtt, m_flow_id, m_omnidma_enable, m_omnidma_bitmap_size);
}
