...
RdmaClientHelper::RdmaClientHelper (uint16_t pg, Ipv4Address sip, Ipv4Address dip, uint16_t sport, uint16_t dport,
                                    uint64_t size, uint32_t win, uint64_t baseRtt)
{
...
RdmaClientHelper::RdmaClientHelper (uint16_t pg, Ipv4Address sip, Ipv4Address dip, uint16_t sport, uint16_t dport,
  uint64_t size, uint32_t win, uint64_t baseRtt, bool omniDMA_enable, uint16_t bitmap_size)
{
m_factory.SetTypeId (RdmaClient::GetTypeId ());
SetAttribute ("PriorityGroup", UintegerValue (pg));
SetAttribute ("SourceIP", Ipv4AddressValue (sip));
...
SetAttribute ("OmniDMAEnable", UintegerValue (omniDMA_enable));
SetAttribute ("OmniDMABitmapSize", UintegerValue (bitmap_size));
}
