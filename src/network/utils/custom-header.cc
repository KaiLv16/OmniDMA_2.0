 ...
 */
 ...
 uint32_t CustomHeader::GetSerializedSize (void) const{
	 ...
	 if (headerType & L4_Header){
		 ...
		 else if (l3Prot == 0x12) // OmniUDP
			 len += GetOmniUdpHeaderSize();
 ...
 void CustomHeader::Serialize (Buffer::Iterator start) const{
   ...
   if (headerType & L4_Header){
	   if (l3Prot == 0x6){ // TCP
		   ...
		   i.WriteHtonU32 (udp.flow_id);
		   i.WriteHtonU16 (udp.omni_type);
		   i.WriteHtonU16 (udp.pg);
		   udp.ih.Serialize(i);
		   i.WriteHtonU32 (udp.adamap_id);
	   } else if (l3Prot == 0x12){ // OmniUDP
			// udp header
			i.WriteHtonU16 (omni_udp.sport);
			i.WriteHtonU16 (omni_udp.dport);
			i.WriteHtonU16 (omni_udp.payload_size);
			i.WriteHtonU16 (0);
			// SeqTsHeader
			i.WriteHtonU32 (omni_udp.seq);
			i.WriteHtonU16 (omni_udp.flow_id);
			i.WriteHtonU16 (omni_udp.omni_type);
			i.WriteHtonU16 (omni_udp.pg);
			omni_udp.ih.Serialize(i);
			i.WriteHtonU32 (omni_udp.adamap_id);
	   ...
	   }else if (l3Prot == 0xFC || l3Prot == 0xFD || l3Prot == 0xF8 || l3Prot == 0xFA){ // ACK or NACK or FastCNP
		   i.WriteU16(ack.sport);
		   i.WriteU16(ack.dport);
		   i.WriteU16(ack.flags);
		   i.WriteU16(ack.pg);
		   i.WriteU16(ack.flow_id);
		   i.WriteU16(ack.omni_type);
		   i.WriteU32(ack.seq);
 ...
 //   printf("\nParse Packet start\n");
   ...
   if (headerType & L4_Header){
	   if (l3Prot == 0x6){ // TCP
		   ...
		   udp.omni_type = i.ReadNtohU16 ();
		   udp.flow_id = i.ReadNtohU16 ();
		   udp.pg = i.ReadNtohU16 ();
		   if (getInt)
			   udp.ih.Deserialize(i);
		   udp.adamap_id = i.ReadNtohU32();
		   l4Size = GetUdpHeaderSize();
	   }else if (l3Prot == 0x12){ // UDP + SeqTsHeader
			i = start;
			i.Next(l2Size + l3Size);
			// udp header
			omni_udp.sport = i.ReadNtohU16 ();
			omni_udp.dport = i.ReadNtohU16 ();
			if (brief){
				i.Next(4);
			}else{
				omni_udp.payload_size = i.ReadNtohU16();
				i.Next(2);
			}
			// SeqTsHeader
			omni_udp.seq = i.ReadNtohU32 ();
			omni_udp.omni_type = i.ReadNtohU16 ();
			omni_udp.flow_id = i.ReadNtohU16 ();
			omni_udp.pg = i.ReadNtohU16 ();
			if (getInt)
			omni_udp.ih.Deserialize(i);
			omni_udp.adamap_id = i.ReadNtohU32();
			l4Size = GetOmniUdpHeaderSize();
		}else if (l3Prot == 0xFF){
		   cnp.qIndex = i.ReadU8();
		   cnp.fid = i.ReadU16();
		   ...
		   cnp.total = i.ReadU16();
		   l4Size = 8;
	   }else if (l3Prot == 0xFC || l3Prot == 0xFD || l3Prot == 0xF8 || l3Prot == 0xFA){ // ACK or NACK
		   	ack.sport = i.ReadU16();
		   	ack.dport = i.ReadU16();
		   	ack.flags = i.ReadU16();
		   	ack.pg = i.ReadU16();
		   	ack.flow_id = i.ReadU16();
		   	ack.omni_type = i.ReadU16();
		   	ack.seq = i.ReadU32();
 ...
 uint32_t CustomHeader::GetUdpHeaderSize(void){
	 return 8 + sizeof(udp.pg) + sizeof(udp.seq) + sizeof(udp.omni_type) + sizeof(udp.flow_id) 
	         + IntHeader::GetStaticSize() + sizeof(udp.adamap_id);
 }

 uint32_t CustomHeader::GetOmniUdpHeaderSize(void){
	return 8 + sizeof(omni_udp.pg) + sizeof(omni_udp.seq) + sizeof(omni_udp.omni_type) + sizeof(omni_udp.flow_id)
			 + IntHeader::GetStaticSize() + sizeof(omni_udp.adamap_id);
}
