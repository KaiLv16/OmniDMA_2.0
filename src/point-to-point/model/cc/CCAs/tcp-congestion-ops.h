 ...
 *
 */
#ifndef RdmaCONGESTIONOPS_H
#define RdmaCONGESTIONOPS_H

#include "rdma-rate-ops.h"
 ...
 *
 * The various congestion control algorithms, also known as "Rdma flavors".
 */
 ...
 * variables are maintained in the RdmaSocketState class, while subclasses of
 * RdmaCongestionOps operate over an instance of that class.
 *
...
class RdmaCongestionOps : public Object
{
  public:
     ...
     */
    RdmaCongestionOps(const RdmaCongestionOps& other);
     ...
     */
    virtual void Init(Ptr<RdmaSocketState> tcb [[maybe_unused]])
    {
    }
     ...
     *
     * Is guaranteed that the congestion control state (\p RdmaAckState_t) is
     * changed BEFORE the invocation of this method.
     * The implementer should return the slow start threshold (and not change
     * it directly) because, in the future, the Rdma implementation may require to
     ...
     */
    virtual uint32_t GetSsThresh(Ptr<const RdmaSocketState> tcb, uint32_t bytesInFlight) = 0;
     ...
     */
    virtual void IncreaseWindow(Ptr<RdmaSocketState> tcb, uint32_t segmentsAcked);
     ...
     * @param rtt last rtt
     */
    virtual void PktsAcked(Ptr<RdmaSocketState> tcb, uint32_t segmentsAcked, const Time& rtt);
     ...
     *
     * @code
     *   m_congestionControl->CongestionStateSet (m_tcb, RdmaSocketState::CA_RECOVERY);
     *   m_tcb->m_congState = RdmaSocketState::CA_RECOVERY;
     * @endcode
     *
     * @param tcb internal congestion state
     * @param newState new congestion state to which the Rdma is going to switch
     */
    virtual void CongestionStateSet(Ptr<RdmaSocketState> tcb,
                                    const RdmaSocketState::RdmaCongState_t newState);
     ...
     */
    virtual void CwndEvent(Ptr<RdmaSocketState> tcb, const RdmaSocketState::RdmaCAEvent_t event);
     ...
     * from the perspective of RdmaSocketBase, the behavior is different if
     ...
     */
    virtual void CongControl(Ptr<RdmaSocketState> tcb,
                             const RdmaRateOps::RdmaRateConnection& rc,
                             const RdmaRateOps::RdmaRateSample& rs);

    // Present in Linux but not in ns-3 yet:
    /* call when ack arrives (optional) */
    virtual void InAckEvent(Ptr<RdmaSocketState> tcb, uint32_t flags);
    /* new value of cwnd after loss (optional) */
    //     u32  (*undo_cwnd)(struct sock *sk);
    /* hook for packet ack accounting (optional) */
    //     void (*pkts_acked)(struct sock *sk, u32 ext, int *attr, union Rdma_cc_info *info);
 ...
 * @see IncreaseWindow
 */
class RdmaNewReno : public RdmaCongestionOps
{
  public:
    /**
     * @brief Get the type ID.
     ...
     */
    RdmaNewReno(const RdmaNewReno& sock);

    ~RdmaNewReno() override;

    std::string GetName() const override;

    void IncreaseWindow(Ptr<RdmaSocketState> tcb, uint32_t segmentsAcked) override;
    uint32_t GetSsThresh(Ptr<const RdmaSocketState> tcb, uint32_t bytesInFlight) override;
    Ptr<RdmaCongestionOps> Fork() override;

  protected:
    virtual uint32_t SlowStart(Ptr<RdmaSocketState> tcb, uint32_t segmentsAcked);
    virtual void CongestionAvoidance(Ptr<RdmaSocketState> tcb, uint32_t segmentsAcked);
};

} // namespace ns3

#endif // RdmaCONGESTIONOPS_H
