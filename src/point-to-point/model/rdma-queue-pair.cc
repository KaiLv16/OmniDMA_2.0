...
void RdmaQueuePair::SetOmniDMAEnable(bool v) {omniDMA.m_omnidma_enabled = v; }
void RdmaQueuePair::SetOmniDMABitmapSize(uint16_t v){omniDMA.m_omnidma_bitmap_size = v; }
...
uint64_t RdmaQueuePair::GetBytesLeft() {
    ...
    if (omniDMA.m_omnidma_enabled) {
        uint32_t normal_data = m_size >= snd_nxt ? m_size - snd_nxt : 0;
        uint32_t omni_data = omniDMA.adamap_sender.NumRetransPkts() * 1000;
        return normal_data + omni_data;
    }
