 ...
 */
 ...
 #ifndef SEQ_OMNI_TS_HEADER_H
 #define SEQ_OMNI_TS_HEADER_H
  ...
  * a 64bits time stamp.
  */
 class SeqOmniTsHeader : public Header
 {
 public:
 SeqOmniTsHeader ();
   ...
   void SetOmniType (uint16_t type);
   uint16_t GetOmniType () const;
 
   void SetFlowId (uint16_t id);
   uint16_t GetFlowId () const;

   void SetAdamapId (uint16_t id);
   uint16_t GetAdamapId () const;
 ...
 private:
   ...
   uint16_t m_omni_type;  // 我加的
   uint16_t m_pg;
   uint16_t m_adamap_id;  // 我加的
 public:
   IntHeader ih;
 };
