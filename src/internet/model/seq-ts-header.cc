 ...
 */
 ...
 void
 SeqTsHeader::SetOmniType (uint16_t type)
 {
   m_omni_type = type;
 }
 uint16_t
 SeqTsHeader::GetOmniType (void) const
 {
   return m_omni_type;
 }
 ...
 void
 SeqTsHeader::SetAdamapId (uint32_t Id)
 {
  m_adamap_id = Id;
 }

 uint32_t
 SeqTsHeader::GetAdamapId (void) const
 {
   return m_adamap_id;
 }
 ...
 uint32_t SeqTsHeader::GetHeaderSize(void){
   return 14 + IntHeader::GetStaticSize();
 }
