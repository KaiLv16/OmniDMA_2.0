...
Generate both patterns with default settings (inter-arrival avg=1ms, var=1ms, flow_size=1000000000, base_time=2.0s):
...
Explicitly override the start-time base offset:
...
- `--avg-ms`: mean inter-arrival time in ms (default: `1.0`)
- `--var-ms`: inter-arrival variance in ms^2 (default: `1.0`)
- `--flow-size`: flow size in bytes (default: `1000000000`)
- `--base-time-s`: start-time offset in seconds (default: `2.0`)
- `--seed`: random seed (optional)
- `--output-dir`: output directory (default: repo `config/`)

Note: the first flow starts at `--base-time-s`, and later flows are generated by accumulating sampled inter-arrivals.

Note: to allow independent control of mean and variance, inter-arrivals are sampled from a Gamma distribution (instead of a pure exponential/Poisson inter-arrival process).
