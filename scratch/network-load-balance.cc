...
uint64_t one_hop_delay = 1000000;  // nanoseconds
...
FILE *switch_drop_output = NULL;
FILE *omniDMA_event_output = NULL;
...
std::string switch_drop_output_file = "switch_drop_output_file.txt";
std::string omniDMA_event_output_file = "omniDMA_event_output_file.txt";
...
uint32_t has_win = 1;
bool self_define_win = false;
uint32_t self_win_bytes = 0;
uint32_t global_t = 1;
...
void stop_simulation_middle() {
    ...
    if (Settings::cnt_finished_flows >= target_flow_num) {
        std::cout << "Settings::cnt_finished_flows = " << Settings::cnt_finished_flows
                  << " target_flow_num = " << target_flow_num << std::endl;
...
int main(int argc, char *argv[]) {
...
#endif
    {
        ...
        while (!conf.eof()) {
            ...
            if (key.compare("FLOW_INPUT_FILE") == 0) {
            ...
            } else if (key.compare("SWITCH_DROP_OUTPUT_FILE") == 0) {
                conf >> switch_drop_output_file;
                std::cerr << "SWITCH_DROP_OUTPUT_FILE\t\t" << switch_drop_output_file << '\n';
            }
            else if (key.compare("OMNIDMA_EVENT_OUTPUT_FILE") == 0) {
                ...
                var_win = v;
                std::cerr << "VAR_WIN\t\t" << v << '\n';
            } else if (key.compare("SELF_DEFINE_WIN") == 0) {
                uint32_t v;
                conf >> v;
                self_define_win = v;
                std::cerr << "SELF_DEFINE_WIN\t\t" << v << '\n';
            } else if (key.compare("SELF_WIN_BYTES") == 0) {
                uint32_t v;
                conf >> v;
                self_win_bytes = v;
                std::cerr << "SELF_WIN_BYTES\t\t" << v << '\n';
            }       
            ...
            } else if (key.compare("ENABLE_OMNIDMA") == 0) {
                bool v;
                conf >> v;
                enable_omnidma = v;
    ...
    for (uint32_t i = 0; i < node_num; i++) {
        if (n.Get(i)->GetNodeType() == 0) {  // is server
            ...
            rdmaHw->SetAttribute("IrnRtoHigh", TimeValue(MicroSeconds(1000)));  // 1930
            rdmaHw->SetAttribute("IrnRtoLow", TimeValue(MicroSeconds(320)));   // 454
            ...
            // 长距离测试(GBN)：
            // rdmaHw->SetAttribute("L2Timeout", TimeValue(MilliSeconds(300)));
